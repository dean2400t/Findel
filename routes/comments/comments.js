...
router.get('/retrieve_comments',async function(req, res) {
    ...
    var object_collection_name = req.query.object_collection_name;
    if (!object_id)
        return res.status(400).send('No object_id was sent')
    if (!object_collection_name)
        return res.status(400).send('No object_collection_name was sent')
    var token=req.headers['findel-auth-token'];
    var userID= checkAuthAndReturnUserID(token);
    return await retrieve_comments(object_id, object_collection_name, userID, res);
 });
...
router.post('/add_comment', auth, async function(req, res) {
    ...
    var object_collection_name = req.body.object_collection_name;
    ...
    return await add_comment(text, object_id, object_collection_name, root_comment_id, req.user._id, res)
});
