...
async function best_pages_results(this_of_searchPage, rk_results)
{
    ...
    for (var index=0; index<top_pages.length && num_of_top_pages_used<num_of_top_pages_to_use; index++)
        ...
        {
            await axios.get("/api/pages_to_topics/web_scrape/?pageID="+top_pages[index].pageID + "&edgeID=" + top_pages[index].edgeID + "&force_scrape=true",{
            })
            .then(async (result) => {
                await hash_and_search_page(top_pages[index], result.data.pageHTML, this_of_searchPage); 
